...
// @noframes
// @run-at document-end
// @namespace https://greasyfork.org/users/766861
// ==/UserScript==
		...
		if ((self != top) || (document.title.match(/404\s*?Not\s*?Found/i) != null)) {
			...
			try {
				...
				function jxbvipobj() {
									...
									(function(window, undefined) {
										...
										(function(window, undefined) {
											...
											function matcherFromTokens(tokens) {
												var checkContext, matcher, j, len = t|ens.length,
					...
					if (iqiyialiwaiobj) {} else if (pcliwaiobj || sjliwaiobj || jxjkobj || mmkjjtjobj || objj.match(/(?:douban|bilibili|acfun|douban|yinyuetai|kuwo|kugou|m1907|qq|xiami|migu|ximalaya|app-echo|1ting|lrts|baidu|wbdy|u23c|ak1080|web3v|pgplayer)\./)) {
						...
						if (jxbpclwobj) {
							...
							cssobj('[class^="mo-foot-info"]');
							if (obj.match(/\/vodplay\//)) {
								jxqtwzobj();
								jxzdqpobj();
								zdbt();
								cssobj('div.maomibtn a.maomi+ul>li:not([style]):not([id]),div[class="mo-cols-info mo-cols-xs2 mo-text-muted mo-coxs-right"],div[class="mo-cols-lays mo-back-white mo-part-round mo-cols-rows mo-part-over"]+div[class="mo-cols-lays mo-back-white mo-part-round mo-cols-rows mo-part-over mo-java-play mo-java-page"]');
							} else {}
							setTimeout(() => {
							...
							if (obj.match(/^https?:\/\/www\.u23c\.com\/label\/jxplayer\.html\?url=http/)) {
								...
								setTimeout(function() {
									qpobj('iframe[width="100%"][src*="=http"]');
								}, 1234);
							} else if (obj.match(/^https?:\/\/www\.pgplayer\.com\/test\/index\.html\?url=http/)) {
								diyobj('<style>body>:not([class="am-container"]),[class="am-container"]>:not(iframe){display:none!important;max-width:0!important;max-height:0!important;overflow:hidden!important;position:absolute;left:-102030px}html,body{background-color:black}</style>');
								document.querySelector('input[placeholder*="视频播放地址"]').value = window.location.search.substr(5), setTimeout(function() {
									document.querySelector('span>button[type="submit"]').click();
								}, 123);
								setTimeout(function() {
									qpobj('#player');
								}, 1234);
					...
					function jxbmmanzxobj() {
						...
						if (obj.match(/^https?:\/\/(?:greasyfork|sleazyfork)\.org\/.+?\/(?:users\/|scripts(?:\/\d+?-[^\/]+?(?:\/code)?$|\/by-site\/|$|\?))/) || obj.match(/^https?:\/\/(?!.+?\.(?:og[gv]|m(?:p[34]|4a)|w(?:ebm|av)|flv)($|\?.+))(?:music\.163\.com\/|y\.music\.163\.com\/m\/(?:song|dj)\?id=|(?:(?:bd|www)\.)?kuwo\.cn\/|m\.kuwo\.cn\/newh5|www\.kugou\.com\/(?:song|mvweb)\/|m3ws\.kugou\.com\/(?:kgsong|mv)\/|m\.kugou\.com\/|(?:i\.)?y\.qq\.com\/|(?:www|h|m)\.xiami\.com\/|(?:m\.)?music\.migu\.cn\/|5sing\.kugou\.com\/|(?:www|m)\.ximalaya\.com\/|kg\.qq\.com\/node\/|www\.app-echo\.com\/|radio\.sky31\.com\/|www\.1ting\.com\/|www\.lrts\.me\/playlist)/)) {
							if (objj.match(/music\.163\./)) {
								...
								btn.addEventListener('click', function() {
									...
									} else if (objj.match(/(?:greasyfork|sleazyfork)\.org/)) {
										try {
											if (localStorage.getItem("jxbjkyhj") == '1') {
												localStorage.setItem('jxbjkyhj', '0');
												alert('已经开启监控 当前网站脚本 的 内置优惠卷 功能');
												location.reload();
											} else {
												localStorage.setItem('jxbjkyhj', '1');
												alert('已经关闭监控 当前网站脚本 的 内置优惠卷 功能');
												location.reload();
											}
										} catch (err) {}
									} else if (objj.match(/1ting\./)) {
						...
						if (obj.match(/^https?:\/\/(?:greasyfork|sleazyfork)\.org\/.+?\/(?:users\/|scripts(?:\/\d+?-[^\/]+?(?:\/code)?$|\/by-site\/|$|\?))/)) {
							if (obj.match(/\/(?:scripts(?:\?(?:page|q)=[^\/]+?)?$|users\/\d+?-[^\/]+$)/) && localStorage.getItem("jxbjkyhj") != '1') {
								jqjs();
								$('ol a[class*="script-link"]').each((index, item) => {
									item.addEventListener('click', () => {
										localStorage.setItem('jxbjbjkyhjobj', '1');
									});
								});
								(function() {
									if (window.MutationObserver) {
										const forkyhj = document.querySelectorAll('li[data-script-name]:not([data-script-id="398195"])');
										for (let forkyhji = 0; forkyhji < forkyhj.length; forkyhji++) {
											const forkyhjmsa = forkyhj[forkyhji].querySelectorAll('article *');
											for (let forkyhjia = 0; forkyhjia < forkyhjmsa.length; forkyhjia++) {
												if (forkyhjmsa[forkyhjia].innerText.match(/(?:比[^价]*?价|查[^券]*?券|优[^惠]*?惠|购[^物]*?物|返[^利]*?利|淘[^宝]*?宝|天[^猫]*?猫|京[^东]*?东|苏[^宁]*?宁|拼[^多]*?多多)/)) {
													forkyhj[forkyhji].setAttribute("style", "display:none!important;max-width:0!important;max-height:0!important;overflow:hidden!important;position:absolute;left:-102030px;")
												}
											}
										}
									} else {
							...
							} else {} if (obj.match(/\/scripts\/(?!398195)\d+?-[^\/]+?\/code$/) && localStorage.getItem("jxbjkyhj") != '1') {
								try {
									if (document.querySelector('pre')) {
										const prea = document.querySelector('pre');
										if (/\n[ \t]*\/\/[ \t]*@(?:include|match)[ \t]+.+?\.(?:jd|taobao|tmall)\./.test(prea.textContent)) prea.parentNode.insertBefore((() => {
											localStorage.removeItem('jxbjbjkyhjobj');
											window.location.href = document.querySelector("#site-name>a").href;
										})(), prea)
									}
								} catch (err) {}
							} else if (obj.match(/\/scripts\/(?!398195)\d+?-[^\/]+?$/) && localStorage.getItem("jxbjkyhj") != '1') {
								try {
									if (document.querySelector('#script-description')) {
										const preb = document.querySelector('#script-description');
										if (/(?:比[^价]*?价|查[^券]*?券|优[^惠]*?惠|购[^物]*?物|返[^利]*?利|淘[^宝]*?宝|天[^猫]*?猫|京[^东]*?东|苏[^宁]*?宁|拼[^多]*?多多)/.test(preb.textContent)) preb.parentNode.insertBefore((() => {
											if (localStorage.getItem("jxbjbjkyhjobj") == '1') {
												localStorage.removeItem('jxbjbjkyhjobj');
						...
						(function() {
							...
							let aa = 1, bb = '<span><a1 style="display:none"></a1><a2 style="display:none">', cc = '<span><a1 style="color:darkgreen">搜索</a1><a2 style="display:none">', dd = '<span><a1 style="color:darkgreen">跳转</a1><a2 style="display:none">', ee = '<span><a1 style="color:darkgreen">小窗</a1><a2 style="display:none">', eee = '<span style="background-color:cyan"><a1 style="color:darkgreen">Json</a1><a2 style="display:none">', ff = '<span><a1 style="color:darkgreen">大窗</a1><a2 style="display:none">', ii = '<span style="background-color:hotpink"><a1 style="color:darkgreen"></a1><a2 style="display:none">', kk = '<span style="display:grid;background-color:aliceblue"><a1>自用</a1><a2 style="display:none">', jj = '<span style="background-color:darkorange"><a1 style="color:darkgreen"></a1><a2 style="display:none">', gg = '</a2><a3 style="color:#2196F3;zoom:0.8;font-weight:bold;">➽</a3><a4>', hh = '</a2><a3 style="display:none"></a3><a4>', https = [{
								...
								name: jj + aa+++hh + "番茄",
								url: "https://jiexi.8b5q.cn/player/jx.php?url="
							}, {
								name: jj + aa+++hh + "OK",
								url: "https://okjx.superchen.top/?url="
							}, {
								name: jj + aa+++hh + "暮光影视",
																				...
																				if (!document.querySelector("Youtube")) {
																					...
																					maomizdjxcobj = setInterval(function() {
																						if (document.querySelector('div.maomibtn>ul>li>ul>li>a[href*="//www.youtube.com"]') != null) {
																							try {
																								if (document.title.match(/第\s*?\d+?\s*?集/) != null) {
																									window.open(document.querySelector('div.maomibtn>ul>li>ul>li>a[href*="//www.youtube.com"]').href + '+' + document.title.match(/第\s*?\d+?\s*?集/), "_blank");
																								} else {
					...
					try {
						if (mgzdgq == 1 || mgzdgq == 2 || vipzdjx == 1 || vipzdjx == 2 || localStorage.getItem("zdpddy") == '1' || document.querySelector("\u6c5f\u5c0f\u767d\u7535\u8111\u7f51\u9875") && obj.match(/^https?:\/\/liuliuyy\.com\/player\/play.html\?url=http/)) {
							function panduanshebeiobj() {
								...
								function zdpddyobj() {
									...
									function jiangxiaobaiviphyzdpd() {
										if (obj.match(/^https?:\/\/liuliuyy\.com\/player\/play.html\?url=http/)) {
							...
							try {
								if (document.querySelector("\u6c5f\u5c0f\u767d\u7535\u8111\u7f51\u9875") && obj.match(/^https?:\/\/liuliuyy\.com\/player\/play.html\?url=http/)) {
									panduanshebeiobj();
								} else {
									...
									if (pcliwaiobj && !objj.match('ixigua\.')) {
										(function() {
											try {
												...
												localStorage.setItem("httpsjk", createRandom2(1, document.querySelectorAll("ul#httpsvipul>li").length - 28, document.querySelectorAll("ul#httpsvipul>li").length - 10));
												setTimeout(function() {
													try {
														$("ul#httpsvipul>li>span").css("backgroundImage", "");
														document.querySelector("ul#httpsvipul>li:nth-child(" + localStorage.getItem("httpsjk") + ")>span").style.backgroundImage = "url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAkAAAAMCAMAAACHgmeRAAAABGdBTUEAALGPC/xhBQAAAAFzUkdCAK7OHOkAAAAbUExURX//AIH8BobyGoT/CZP/J4nxIIL5C5T8LJj0PW2hJpgAAAAeSURBVAjXY2BlY4QABiZGBiigNouVGQoYONhZIAAAEpYAdsB2b4AAAAAASUVORK5CYII=)";
									...
									if (pcliwaiobj && !objj.match('ixigua\.')) {
										(function() {
											try {
												...
												localStorage.setItem("httpsjk", createRandom2(1, 4, document.querySelectorAll("ul#httpsvipul>li").length - 28));
												setTimeout(function() {
													try {
														$("ul#httpsvipul>li>span").css("backgroundImage", "");
														document.querySelector("ul#httpsvipul>li:nth-child(" + localStorage.getItem("httpsjk") + ")>span").style.backgroundImage = "url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAkAAAAMCAMAAACHgmeRAAAABGdBTUEAALGPC/xhBQAAAAFzUkdCAK7OHOkAAAAbUExURX//AIH8BobyGoT/CZP/J4nxIIL5C5T8LJj0PW2hJpgAAAAeSURBVAjXY2BlY4QABiZGBiigNouVGQoYONhZIAAAEpYAdsB2b4AAAAAASUVORK5CYII=)";
